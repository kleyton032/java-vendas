...
public class VendasBean {
	...
	public void adicionar(Produto produto) {
		...
		if (posicaoEncontrada < 0) {
			item.setQuantidade_item(1);
			item.setValor_parcial(produto.getPreco());
			itens.add(item);
		} else {

			Item itemTemp = itens.get(posicaoEncontrada);
			item.setQuantidade_item(itemTemp.getQuantidade_item() + 1);
			item.setValor_parcial(produto.getPreco().multiply(new BigDecimal(item.getQuantidade_item())));
	...
	public void salvar() {
		try {
			 ...
			 Long codigoVenda = vdao.salvarVenda(vendaCadastro);
